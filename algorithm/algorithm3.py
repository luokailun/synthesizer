...
def __decide_update_model(model, state_type, Goal, player, strategy_list, INC=3):
	"""
		...
		@param		strategy_list   winning strategy that generated for instances games
	...
	if right_turn is False:
		...
		# if the universe of model has not been generated strategy before, generate for it
		if strategy.find_universe_in_strategies(universe, strategy_list) is False:
			# generate initial models for the universe
			init_models = strategy.get_init_models_with_universe(universe)
			# generate strategy  for each initial model
			strategy_list.extend( [ strategy.check_and_get_strategy(m, Goal, player) for m in init_models] )
		# check whether a model is in strategies
		if strategy.find_model_in_strategies(model, strategy_list) is True: 
			print('**** Win! Include it by next state\n')
			# get the next models for the model in the strategy
			next_include_models = strategy.find_next_models(model, strategy_list)
		else:
			print('**** Not in any strategy! Exclude it\n')
			current_exclude_models = [model]
...
def structure_regress_until_convergence(two_state_structure, pred_list, End,  Goal, player, strategy_list):
	"""
	...
	while True:
		...
		if negative_model1 is True and negative_model2 is True:
			...
			if negative_model1 is not True:
				e_neg_model_list, a_pos_model_list = __decide_update_model(negative_model1, 'E', Goal, player, strategy_list)
			if negative_model2 is not True:
				a_neg_model_list, e_pos_model_list = __decide_update_model(negative_model2, 'A', Goal, player, strategy_list)
...
def synthesis(Init, End, Goal, player, pred_list):
	...
	while n>0:
		...
		new_two_state_structure = structure_regress_until_convergence(two_state_structure, pred_list, End,  Goal, player, strategy_list)
		...
		# If the invariant is not necessary, update the formulas using the counterexamples.
			...
			universe, assignment, default_value = positive_model
			# if the universe of model has not been generated strategy before, generate for it
			if strategy.find_universe_in_strategies(universe, strategy_list) is False:
				# generate strategy for the initial model
				strategy_list.append(strategy.check_and_get_strategy(positive_model, Goal, player))
			# get the next models for the model in the strategy
			progress_model_list = strategy.find_next_models(positive_model, strategy_list)

			print('DS0 progressed updated model:%s\n'%('\n'.join([str(m) for m in progress_model_list])))
			print(format_output.format_outputs(progress_model_list, 'Ch'))
			print('---------------------------------------------------')
			fstructure1, fstructure2 = new_two_state_structure
			fstructure1 = __update_structure('q1',fstructure1,  [], [positive_model], pred_list)
			fstructure2 = __update_structure('q2', fstructure2, [], progress_model_list, pred_list)
			two_state_structure = fstructure1, fstructure2
